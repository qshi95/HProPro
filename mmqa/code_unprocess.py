...
from prompt.check_prompt import CHECK_PROMPT, CHECK_SAME_PROMPT
from prompt.extract_info import EXTRACT_INFO
...
def solve(table) -> str:
    for index, row in table.iterrows():
        if row['Name'][0] == 'FMG Stadium Waikato':
            sport = extract_info(row['Type'][1], 'What sport is played here?')
            location = extract_info(row['Location'][1], 'number of territorial authorities')
            if location == '11':
                return sport
    return 'NOT_AVAILABLE'
